...
export const parseOZExcel = (jsonData: any[][], userId?: string): Flight[] => {
  ...
  if (firstRow && Array.isArray(firstRow)) {
    ...
    // 첫 번째 행이 "BRIEFING INFO"로 시작하는 경우만 브리핑 형식으로 인식
    if (firstRowText.startsWith('BRIEFING INFO')) {
  ...
  try {
    ...
    for (const row of headerScanRows) {
      ...
      // 스케줄 본문 라벨
      if (upper.includes('DATE')) idxDate = upper.indexOf('DATE');
      if (upper.includes('FLIGHT')) idxFlight = upper.indexOf('FLIGHT');
      if (upper.includes('SHOW UP')) idxShowUp = upper.indexOf('SHOW UP');
      if (upper.includes('SECTOR')) idxSector = upper.indexOf('SECTOR');
      if (upper.includes('STD')) idxSTD = upper.indexOf('STD');
      if (upper.includes('STA')) idxSTA = upper.indexOf('STA');
  ...
  dataRows.forEach((row, rowIndex) => {
    ...
    if (rowIndex === 0) {
      console.log('🔍 첫 번째 행 컬럼 데이터:', {
        rowLength: row.length,
        columns: row.map((col, idx) => ({ index: idx, value: col }))
      });
    }
    ...
    if (hasCockpitCrewInfo) {
      // 디버깅용 로그 (테스트용)
      console.log('🔍 월 비행스케줄 조종사 파싱:', { 
        flightNumber, 
        hasCockpitCrewInfo, 
        crewMember,
        isCockpitCrew: isCockpitCrew(crewMember),
        lastFlightKey: `${lastDate}-${flightNumber || route}`,
        rowIndex,
        rawRow: [row[idxEmpl], row[idxName], row[idxRank], row[idxPosnType], row[idxPosn]], // EMPL~POSN 원본 데이터
        nameCheck: crewMember.name,
        nameTrimmed: crewMember.name?.trim(),
        nameNotEmpty: crewMember.name && crewMember.name.trim() !== ''
      });
    } else {
      // 크루 정보가 없는 경우도 로그로 확인
      console.log('❌ 크루 정보 없음:', {
        flightNumber,
        crewMember,
        hasCockpitCrewInfo,
        rowIndex,
        rawRow: [row[idxEmpl], row[idxName], row[idxRank], row[idxPosnType], row[idxPosn]]
      });
    }
    ...
    if (isFlightRow || isSpecialScheduleRow || isStbyRow || (lastFlightKey && !flightNumber && hasCockpitCrewInfo)) {
      ...
      if (existingFlight) {
        console.log('🔍 기존 비행편 발견:', {
          flightKey: lastFlightKey,
          existingCrewCount: existingFlight.crew.length,
          hasCockpitCrewInfo,
          isCockpitCrew: isCockpitCrew(crewMember),
          crewMember
        });
        ...
        if (hasCockpitCrewInfo && isCockpitCrew(crewMember)) {
          ...
          if (!isDuplicate) {
            ...
            console.log('✅ 기존 비행편에 조종사 추가:', crewMember.name, '총 조종사 수:', existingFlight.crew.length);
          } else {
            console.log('⚠️ 중복된 조종사 제외:', crewMember.name);
          }
        } else {
          console.log('❌ 기존 비행편에 크루 추가 실패:', {
            hasCockpitCrewInfo,
            isCockpitCrew: isCockpitCrew(crewMember),
            crewMember
          });
        ...
        if (hasCockpitCrewInfo && isCockpitCrew(crewMember)) {
          ...
          console.log('✅ 새 비행편에 조종사 추가:', crewMember.name, '총 조종사 수:', newFlight.crew.length);
        } else {
          console.log('❌ 새 비행편에 크루 추가 실패:', {
            hasCockpitCrewInfo,
            isCockpitCrew: isCockpitCrew(crewMember),
            crewMember
          });
    ...
    // Crew 정보만 있는 행 (비행편 정보가 없는 경우) - 조종사 정보만 처리
      ...
      if (existingFlight && isCockpitCrew(crewMember)) {
        ...
        if (!isDuplicate) {
          existingFlight.crew.push(crewMember);
          console.log('✅ Crew만 있는 행에서 조종사 추가:', crewMember.name, '총 조종사 수:', existingFlight.crew.length);
        } else {
          console.log('⚠️ Crew만 있는 행에서 중복된 조종사 제외:', crewMember.name);
        }
      }
    }
  });
  ...
  if (isBriefingFormat && briefingFlightDate && briefingFlightNumber) {
    console.log('🔍 BRIEFING INFO 병합 시작:', {
      briefingFlightDate,
      briefingFlightNumber,
      briefingCockpitCrewCount: briefingCockpitCrew.length,
      briefingCabinCrewCount: briefingCabinCrew.length
    });
