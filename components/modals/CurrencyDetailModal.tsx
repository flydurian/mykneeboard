        ...
        return (
            <div className="mb-6">
                <div className="flex items-center justify-between mb-3">
                    <h3 className="text-sm font-semibold text-slate-300">월별횟수(최근6개월)</h3>
                    ...
                    <svg width={width} height={height} className="mx-auto drop-shadow-sm">
                        ...
                        {monthlyData.map((data, index) => (
                            <text
                                ...
                                y={height - 8}
                                textAnchor="middle"
                                className="text-xs font-medium fill-current text-slate-400"
                            >
                                {data.displayMonth}
                            </text>
                        ))}
    ...
    return (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4 pt-safe" onClick={onClose}>
            <div className="glass-panel rounded-2xl shadow-xl w-full max-w-md p-6 relative animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
                ...
                <h2 className="text-xl font-bold text-white mb-4">{data.title} 최근 이력</h2>
                ...
                {data.events.length > 0 ? (
                    <>
                        ...
                        {/* 최근 이력 목록 */}
                        <div>
                            <h3 className="text-sm font-semibold text-slate-300 mb-2">최근 기록</h3>
                            <ul className="space-y-2">
                                {data.events
                                    ...
                                    .map(event => (
                                        <li
                                            key={event.id}
                                            className="p-1.5 bg-black/20 rounded-lg cursor-pointer hover:bg-white/10 transition-colors"
                    ...
                    <p className="text-slate-400">최근 90일 내 기록이 없습니다.</p>
                )}
            </div>
        </div>
    );
