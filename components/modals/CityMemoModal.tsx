...
const CityMemoModal: React.FC<CityMemoModalProps> = ({ isOpen, onClose, cityCode, initialMemo, onSave }) => {
    ...
    useEffect(() => {
        if (isEditing) {
            ...
            if (!el) return;
            // 편집 진입 시 현재 memo를 한 번만 주입 (이후 타이핑 중에는 재주입 금지)
    ...
    const handleTextChange = (e: React.FormEvent<HTMLDivElement>) => {
        if (isComposing) return; // 한글 조합 중에는 상태를 갱신하지 않음 (커서/조합 보호)
        const target = e.target as HTMLDivElement;
        // 편집 중에는 상태를 바꾸지 않고 ref만 업데이트 (커서 안정성)
    ...
    // HTML 서식 허용 붙여넣기: 안전하게 허용된 태그/스타일만 유지
    ...
    useEffect(() => {
        if (!isEditing && textareaRef.current && memo !== textareaRef.current.innerHTML) {
            textareaRef.current.innerHTML = memo || `<span class="text-gray-500 dark:text-gray-400">${cityCode} 도시에 대한 메모를 입력하세요...</span>`;
            setTimeout(() => {
                adjustTextareaHeight();
            }, 0);
        }
    ...
    const handleKeyDown = (e: React.KeyboardEvent<HTMLDivElement>) => {
        if (e.key === 'Enter') {
            // Shift+Enter는 줄바꿈, Enter 단독도 줄바꿈 허용
    ...
    const changeFontSize = (size: string) => {
        ...
        // 커서를 방금 삽입한 span 뒤로 이동
        range.setStartAfter(span);
        range.setEndAfter(span);
        selection.removeAllRanges();
        selection.addRange(range);
        // 상태는 ref로만 업데이트 (커서 안정성)
        draftMemoRef.current = textareaRef.current.innerHTML;
        setEditLength(draftMemoRef.current.length);
    };
